...
it is also a candidate for deletion in `docs/archive/accidental-complexity-reduction-plan.md`). The
...
- **D1 (owner decision 2026-07-17, `docs/archive/claimsspace-footprint-default-and-scroll-reachability-plan.md`):
...
`docs/architecture/serialization-duplication-reference.md` for where ser/deser details are documented.
...
in `docs/archive/affine-geometry-api-plan.md` (that doc is canonical); this section is the pointer.** Shipped
...
0. **⭐ TOP PERF FOLLOW-UP — §4.4 island buffer cache — ✅ LANDED 2026-07-11 (= item 15 below; its own
   doc `docs/archive/island-buffer-cache-plan.md`).** The buffer is now kept across composites and warped for a
...
14. **Public geometry API unit — ✅ LANDED 2026-07-11 (Phase 5, first banked item), its own doc:**
   `docs/archive/affine-geometry-api-plan.md` (self-contained; top banner now LANDED). Shipped the two-vocabulary
...
15. **⭐ Island buffer cache (completes §4.4; the TOP perf follow-up) — ✅ LANDED 2026-07-11, its own doc:
   `docs/archive/island-buffer-cache-plan.md`** (design LOCKED + executed; §6 = results). Makes §9/§10.9/§10.10's
...
- Rotation animation of a static window: per step = matrix update + damage (oldAABB ∪ newAABB) +
  ...
  `docs/archive/island-buffer-cache-plan.md`): `_refreshIslandBuffer` keeps the content buffer ACROSS composites and
...
(LANDED 2026-07-11, `docs/archive/island-buffer-cache-plan.md`) keeps the buffer across composites and warps it for
